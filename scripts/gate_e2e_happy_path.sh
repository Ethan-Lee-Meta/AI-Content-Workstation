...
latest() {
  # pick the newest matching log (supports repeat mode suffixes)
...
need_file() {
  f="$(latest "$1")"
  if [ -z "$f" ] || [ ! -s "$f" ]; then
    echo "[err] missing/empty log for gate '${1}' (expected tmp/_out_gate_${1}*.txt)"
    echo "[hint] run: bash scripts/gate_all.sh --mode=full"
    return 2
  fi
  echo "[info] using log: $f"
  echo "$f"
  return 0
}
...
# happy path evidence anchors (from AC gates)
