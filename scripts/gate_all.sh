...
gen_evidence_full() {
...

  {
    ...
    echo '```text'
    for label in health_contract_check request_id_propagation_check openapi_reachable api_smoke provider_adapter web_routes ac_001 ac_002 ac_003 ac_004 e2e_happy_path shots_api shots_ui; do
...
bash scripts/gate_export_import.sh
  ...
  bash scripts/gate_runs_trace_v11.sh
    ...
    run_gate "export_preview_ui" "scripts/gate_export_preview_ui.sh" "$i" || exit $?

    run_gate "e2e_happy_path" "scripts/gate_e2e_happy_path.sh" "$i" || exit $?
    gen_evidence_full "$i" || exit $?

  # == BATCH-4 UI gates (Settings/Characters) ==
  if [ -x scripts/gate_settings_ui.sh ]; then
    run_gate "settings_ui" bash scripts/gate_settings_ui.sh
  else
    echo "[warn] missing: scripts/gate_settings_ui.sh"
  fi
  if [ -x scripts/gate_characters_ui.sh ]; then
    run_gate "characters_ui" bash scripts/gate_characters_ui.sh
  else
    echo "[warn] missing: scripts/gate_characters_ui.sh"
  fi
  else
